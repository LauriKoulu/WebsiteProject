...
// these do have api keys in the url but it's not that sensitive information so will leave these here.
...
new Vue({
	el: '#app',
	created: function() {
		// get the score data from database on create
		this.getScore();
	},
	...
	mounted() {
		...
		// this is needed(?). function wouldn't detect other functions and variables without.
		var self = this;
	...
	methods: {
		...
		// this method uses canvas to draw the playing grid
		initGame: function(){
			var c = document.getElementById("myCanvas");
		...
		// called if the player wins the game. function uses axios to update total stats.
		oWin: function() {
		...
		// called when AI wins the game. increases the amount of AI wins in total stats.
		xWin: function() {
			var url = 'https://api.mlab.com/api/1/databases/local_library/collections/totalstats?apiKey=i0tCYutvccPPCzMneb1bq0xN2-GUdgZB';
			// increment wins by 1
		...
		// draws a circle in position determined by parameter drawPos.
		...
		// draws a cross in the position determined by drawPos.
		...
		// called on mouse move. returns mouse coordinates within canvas.
		...
		// function check for winner. only works on 3 by 3 grid. return true if winner is found.
		...
		// called when we want the game to end. updates canvas with game over screen.
		...
		// function adds win/loss/draw to player and updates the score in database.
		...
		// called when it's AI turn to make a move
		// uses random to make a move
		// passes turn to player when done.
		compTurn: function() {
			var moveDone = false;
		...
		// used for getting the exact middle of a grid players mouse is on so we draw at right position.
